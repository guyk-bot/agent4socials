...
**If you see "Invalid Scopes: pages_manage_posts, pages_manage_engagement":** Facebook shows this when your app has not yet added these permissions. (1) In [Meta for Developers](https://developers.facebook.com/) open your app → **App Review** → **Permissions and features**. (2) Search for **pages_manage_posts** and **pages_manage_engagement** and add each to your app (Add to App Review or add to the Facebook Login use case). (3) Reconnect Facebook. Until the permissions are added, the OAuth dialog will reject the scope. **Temporary workaround:** In Vercel set **FACEBOOK_OAUTH_SCOPES** to a reduced list without the invalid ones, e.g. `pages_read_engagement,pages_show_list,pages_messaging,pages_read_user_content,read_insights,business_management`. Then add the two permissions in Meta and remove the env var (or set it to the full scope) and reconnect.

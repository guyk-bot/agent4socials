...
**Insights (views, reach, profile views):** The app requests `instagram_manage_insights` (Instagram via Facebook) and `instagram_business_manage_insights` (Instagram-only login) so account metrics (impressions, reach, profile views) are available. In your Meta app, ensure the **Instagram Graph API** product is added and that **instagram_manage_insights** (and **instagram_business_manage_insights** if using Instagram-only login) are enabled under Permissions and features. If you connected accounts before adding these scopes, disconnect and reconnect so the new permissions are granted; then the dashboard will show Views, Reach, and Profile views (Instagram) or Page visits (Facebook).

...
- **Facebook Page:** Posting to a Page’s feed requires **pages_manage_posts**. Add that permission in [Meta for Developers](https://developers.facebook.com/) (App Review → Permissions and features), then set **FACEBOOK_OAUTH_SCOPES** to the full list including `pages_manage_posts` (see "Invalid Scopes: pages_manage_posts, pages_manage_engagement" below), redeploy, and reconnect. Until then, Connect works but Page posts from the Composer will fail with a permission error.

**If you see "Invalid Scopes: pages_manage_posts":** The default scope no longer includes it. To enable Page posting, add **pages_manage_posts** in the Meta app (App Review → Permissions and features), then set **FACEBOOK_OAUTH_SCOPES** to the full list (see "Invalid Scopes: pages_manage_posts, pages_manage_engagement" above) and reconnect Facebook.

**If you see "Invalid Scopes: pages_manage_posts, pages_manage_engagement":** The app **default** no longer requests these scopes so Connect works without adding them in Meta. **Facebook:** To enable Page posting and comment management, add both permissions in [Meta for Developers](https://developers.facebook.com/) → your app → **App Review** → **Permissions and features**, then in Vercel set **FACEBOOK_OAUTH_SCOPES** to: `pages_read_engagement,pages_show_list,pages_manage_posts,pages_manage_engagement,pages_messaging,pages_read_user_content,read_insights,business_management`, redeploy, and reconnect Facebook. **Instagram via Facebook:** Same two scopes are omitted by default; to request them set **INSTAGRAM_VIA_FACEBOOK_OAUTH_SCOPES** to the full scope list (including `pages_manage_posts,pages_manage_engagement`) and redeploy.

**read_insights (Facebook Page analytics):** Yes, **read_insights** is the permission for Page insights (views, page visits, follower gains). The app requests it by default so the dashboard can show Page analytics. In [Meta for Developers](https://developers.facebook.com/) → your app → **Use cases** → **Pages API** (or **App Review** → **Permissions and features**), ensure **read_insights** is added and "Ready to publish". If you set **FACEBOOK_OAUTH_SCOPES** in Vercel, the list must include `read_insights` or Page views/visits will stay at 0. After the permission is in the app (and approved if Live), **disconnect and reconnect** your Facebook account so the new token includes read_insights (the "0" count in Meta means no one has granted it yet; reconnecting fixes that). If you see **Invalid Scopes: read_insights**, add the permission to the app first, then try connecting again.
...
**Why Total content is 0, no previous posts, and Inbox is empty:** The app needs the **Page** access token for both Instagram and Facebook to sync posts from the platform and to load inbox conversations. If you connected before we stored the Page token, or skipped the "Choose Page" step, you will see:
- **Total content: 0** and no list of posts (sync from platform returns nothing or fails).
- **Inbox:** "Could not load conversations" or timeout (conversations API needs the Page token with `pages_messaging` / `instagram_manage_messages`).
- **Analytics:** Followers and views may still show (from profile/insights), but post-level engagement and "Total content" come from the **synced posts**, which require a successful sync with the Page token.

**Fix:** Reconnect Facebook (and Instagram) from the dashboard sidebar: click **Reconnect** for Facebook, complete the flow, and **choose your Page** when asked. After that, use **Sync posts** on the Analytics → Account tab if Total content is still 0. Inbox should then load (or show "No conversations" if you have none).
...
**App Review → Testing: "0 of 1 API call(s) required":** Before submitting for review, Meta requires at least one successful API call per permission (e.g. **pages_manage_engagement**, **instagram_business_manage_insights**, **instagram_business_manage_comments**). Run `node scripts/run-meta-app-review-tests.js` with `GRAPH_TEST_TOKEN` set (and optionally `INSTAGRAM_TOKEN` + `IG_USER_ID` for the two Instagram tests). Step-by-step manual steps are in **scripts/meta-app-review-api-tests.md**.

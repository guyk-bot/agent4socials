  ...
  try {
    const { error } = await resend.emails.send({
      ...
      html: `
        ...
        <p>${modeText}</p>
        <p><a href="${baseUrl}/posts" style="color:#1C9CFB;font-weight:600">Open Post History</a></p>
      `,
    });
    if (error) {
      const errMsg = typeof error === 'object' && error !== null && 'message' in error ? String((error as { message?: unknown }).message) : String(error);
  ...
  try {
    const { error } = await resend.emails.send({
      ...
      html: `
        ...
        <p>
          <a href="${baseUrl}/posts" style="color:#1C9CFB;font-weight:600">Open Post History</a><br/>
          <a href="${baseUrl}/help/support" style="color:#1C9CFB;font-weight:600">Contact support</a>
        </p>
      `,
    });
    if (error) {

...
import { loadTwitterInboxConversation } from '@/lib/inbox/load-twitter-inbox-conversation';

export type { ConversationUiMessage };

export type LoadInboxConversationResult =
  | {
      ok: true;
...
async function loadMetaInboxConversation(
  userId: string,
  account: AccountSlice,
  conversationId: string
): Promise<LoadInboxConversationResult> {
...
export async function loadInboxConversationMessages(
  account: AccountSlice,
  conversationId: string,
  userId?: string
): Promise<LoadInboxConversationResult> {
  if (account.platform === 'TWITTER') {
    const tw = await loadTwitterInboxConversation(account, conversationId);
    if (!tw.ok) return tw;
    return {
      ok: true,
  ...
  if (account.platform === 'INSTAGRAM' || account.platform === 'FACEBOOK') {
    ...
    return loadMetaInboxConversation(userId, account, conversationId);
  }

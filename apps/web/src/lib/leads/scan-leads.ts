/** Shared lead mining: classify cached inbox comments for buying intent. */
...
export async function scanLeads(userId: string, accountId?: string | null): Promise<ScanLeadsResult> {
  const accounts = await prisma.socialAccount.findMany({
    where: { userId, ...(accountId ? { id: accountId } : {}) },
    select: { id: true, platform: true, username: true },
  });
  if (accounts.length === 0) {
    return { leads: [], scanned: 0, message: 'No connected accounts to scan.' };
  }
  ...
  return { leads, scanned: toScan.length };
}

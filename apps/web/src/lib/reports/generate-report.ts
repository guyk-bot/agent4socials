...
function drawRecommendations(ctx: Ctx, summary: UnifiedSummaryResponse, rollups: PlatformRollup[]) {
  ...
  recs.push('Post consistently (3x per week minimum) and queue ahead with Agent4Socials Scheduler.');
...
function drawFooter(ctx: Ctx, pageNum: number) {
  ...
  pdf.setFontSize(6.5);
  pdf.setFont('helvetica', 'normal');
  pdf.text('Agent4Socials Analytics Report  |  agent4socials.com', 40, pageH - 8);

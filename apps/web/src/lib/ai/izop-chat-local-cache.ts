import type { IzopChatSessionSummary } from '@/lib/ai/izop-chat-sessions';
...
function messagesKey(userId: string, sessionId: string) {
  return `izop_chat_${userId}_${sessionId}`;
}

function listKey(userId: string) {
  return `izop_chat_list_${userId}`;
}

function lastActiveKey(userId: string) {
  return `izop_chat_last_${userId}`;
}
...
export function readLastActiveChatId(userId: string | undefined): string | null {
  ...
  try {
    const raw =
      localStorage.getItem(lastActiveKey(userId)) ??
      localStorage.getItem(`izop_aysop_chat_last_${userId}`);
...
export function readCachedMessages(userId: string | undefined, sessionId: string): CachedChatMessage[] | null {
  ...
  try {
    const raw =
      localStorage.getItem(messagesKey(userId, sessionId)) ??
      localStorage.getItem(`izop_aysop_chat_${userId}_${sessionId}`);
    if (!raw) return null;
...
export function readCachedSessionList(userId: string | undefined): IzopChatSessionSummary[] | null {
  if (!userId) return null;
  try {
    const raw =
      localStorage.getItem(listKey(userId)) ?? localStorage.getItem(`izop_aysop_chat_list_${userId}`);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as IzopChatSessionSummary[];
    return Array.isArray(parsed) ? parsed : null;
  } catch {
    return null;
  }
...
export function writeCachedSessionList(userId: string | undefined, sessions: IzopChatSessionSummary[]): void {
  if (!userId) return;
  try {
...
function deletedIdsKey(userId: string) {
  return `izop_chat_deleted_${userId}`;
}
...
export function readDeletedChatIds(userId: string | undefined): Set<string> {
  ...
  try {
    const raw =
      localStorage.getItem(deletedIdsKey(userId)) ??
      localStorage.getItem(`izop_aysop_chat_deleted_${userId}`);
...
function pendingNewChatKey(userId: string) {
  return `izop_pending_new_chat_${userId}`;
}
...
export function readPendingNewChatId(userId: string | undefined): string | null {
  ...
  try {
    const raw =
      localStorage.getItem(pendingNewChatKey(userId)) ??
      localStorage.getItem(`izop_aysop_pending_new_chat_${userId}`);
    return raw?.trim() ? raw : null;
  } catch {
    return null;
  }

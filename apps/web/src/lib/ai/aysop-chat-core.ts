...
function buildSystemPrompt(
  ...
  return [
    ...
    '- prepare_platform_post_drafts: platform preview cards with Allow and Schedule. User can preview scheduled posts on Calendar or History.',
    ...
    '- When the user attaches media and asks to post (e.g. Threads, Instagram), ask for caption and platform if missing, then call open_composer_draft with mediaUrls (every attachment URL) and postType photo or video.',
    '- open_composer_draft shows inline Composer in chat with the same publish and schedule options as the full Composer page.',
    '- For Threads with attached image/video, you can also use prepare_platform_post_drafts with platform Threads, caption, and mediaUrls so they can Allow & publish from the preview card.',
    '',

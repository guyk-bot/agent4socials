...
import { summarizeWorkspaceAccounts } from '@/lib/ai/izop-workspace-snapshot';
import { runShowAppInChat } from '@/lib/ai/izop-show-app';
import {
...
import {
  IZOP_COMPOSER_HREF,
  buildIzopComposerDraftPayload,
  inferComposerMediaType,
  mediaListFromUrls,
  type IzopComposerDraftPayload,
  type IzopComposerMediaType,
} from '@/lib/composer/izop-composer-draft-bridge';
import { IZOP_CONNECT_PLATFORMS } from '@/lib/ai/izop-connect-platforms';
import { appViewArtifact, type AppViewId } from '@/lib/ai/izop-artifacts';
...
export type { IzopArtifact } from '@/lib/ai/izop-artifacts';
...
export type IzopToolContext = {
  userId: string;
  workspaces?: IzopWorkspaceSnapshot[];
...
function reportToArtifact(report: DashboardAnalyticsReport): IzopArtifact {
  return {
...
async function buildConnectPlatformsArtifact(userId: string): Promise<
  Extract<IzopArtifact, { type: 'connect_platforms' }>
> {
  const accounts = await prisma.socialAccount.findMany({
    where: { userId },
  ...
  const connected = IZOP_CONNECT_PLATFORMS.filter((p) => byPlatform.has(p.platform)).map((p) => ({
    platform: p.platform,
    name: p.name,
    username: byPlatform.get(p.platform) ?? null,
  }));
  const missing = IZOP_CONNECT_PLATFORMS.filter((p) => !byPlatform.has(p.platform)).map((p) => ({
    platform: p.platform,
    name: p.name,
    slug: p.slug,
  }));
...
async function buildRecentInboxArtifact(
...
): Promise<Extract<IzopArtifact, { type: 'inbox_feed' }>> {
  const accounts = await prisma.socialAccount.findMany({
    where: {
      userId,
  ...
  const items: Extract<IzopArtifact, { type: 'inbox_feed' }>['items'] = [];
...
async function buildComposerSessionDraft(
  ...
  artifact: Extract<IzopArtifact, { type: 'composer_session_draft' }>;
  ...
  const draft: IzopComposerDraftPayload = buildIzopComposerDraftPayload({
    platforms,
    caption,
    mediaType,
    mediaList,
  ...
  const artifact: Extract<IzopArtifact, { type: 'composer_session_draft' }> = {
...
async function buildComposerPostDraft(
  ...
  const artifact: Extract<IzopArtifact, { type: 'composer_post_draft' }> = {
...
export const IZOP_TOOL_DEFINITIONS = [
  {
...
export async function runIzopTool(
  name: string,
  args: Record<string, unknown>,
  ctx: IzopToolContext
    ...
    case 'list_brand_workspaces': {
      const workspaces = ctx.workspaces?.length
        ...
        : await (async () => {
            ...
            ] satisfies IzopWorkspaceSnapshot[];
          })();

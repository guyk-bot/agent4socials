...
export type AysopActiveBrandSnapshot = {
  id: string;
  name: string;
} | null;
...
export function readWorkspaceSnapshotFromBrowserStorage(): AysopWorkspaceSnapshot[] {
  ...
  return buildAysopWorkspaceSnapshot(brands, accounts, map);
}

export function readActiveBrandFromBrowserStorage(): AysopActiveBrandSnapshot {
  const brands = readBrandsFromBrowserStorage();
...
export async function resolveChatBrandContext(options: {
...
}): Promise<{ workspaces: AysopWorkspaceSnapshot[]; activeBrand: AysopActiveBrandSnapshot }> {
  ...
  const workspaces = buildAysopWorkspaceSnapshot(brands, accounts, map);

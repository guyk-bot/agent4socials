...
/** Best-effort: create izop chat sessions table if migrations have not run yet. */
export async function ensureIzopChatTable(): Promise<boolean> {
  ...
  inFlight = (async () => {
    try {
      ...
      await runMigration();
      ensured = true;
      console.log('[IzopChat] aysop_chat_sessions table ensured.');
    } catch (e) {
      console.warn('[IzopChat] ensure table failed:', (e as Error).message?.slice(0, 200));
    } finally {
      inFlight = null;
    }
  })();
...
export function resetIzopChatTableEnsure(): void {
  ensured = false;
}

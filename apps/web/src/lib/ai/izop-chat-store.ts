...
import { ensureIzopChatTable, resetIzopChatTableEnsure } from '@/lib/ai/ensure-izop-chat-table';
import {
  previewFromMessages,
...
import { normalizeStoredMessages, hasConversation } from '@/lib/ai/izop-chat-persist';
import type { StoredIzopMessage } from '@/lib/ai/izop-chat-sessions';
import { sessionHasConversation } from '@/lib/ai/izop-chat-sessions';

export type IzopChatSessionRow = IzopChatSessionSummary & {
  messages: StoredIzopMessage[];
};
...
function rowToSession(row: DbRow): IzopChatSessionRow {
...
function summaryFromRow(row: DbRow): IzopChatSessionSummary {
  const messages = normalizeStoredMessages(row.messages);
  return {
    id: row.id,
    title: row.title,
...
function prismaHasIzopModel(): boolean {
  return typeof (prisma as { izopChatSession?: unknown }).izopChatSession !== 'undefined';
}
...
async function updateSessionRaw(
  userId: string,
  id: string,
  data: { title?: string; messages?: StoredIzopMessage[] }
): Promise<DbRow | null> {
...
export async function listIzopChatSessions(userId: string): Promise<IzopChatSessionSummary[]> {
  await ensureIzopChatTable();
  try {
    if (prismaHasIzopModel()) {
      const rows = await prisma.izopChatSession.findMany({
        where: { userId },
...
export async function getIzopChatSession(userId: string, id: string): Promise<IzopChatSessionRow | null> {
  await ensureIzopChatTable();
  try {
    if (prismaHasIzopModel()) {
      const row = await prisma.izopChatSession.findFirst({
        where: { id, userId },
        select: { id: true, title: true, updatedAt: true, createdAt: true, messages: true },
      });
...
export async function createIzopChatSession(
  userId: string,
  title = 'New chat'
): Promise<IzopChatSessionRow> {
  await ensureIzopChatTable();
  try {
    if (prismaHasIzopModel()) {
      const row = await prisma.izopChatSession.create({
    ...
    if (!isMissingTableError(e)) {
      console.error('[IzopChat] prisma create failed, trying raw:', (e as Error).message?.slice(0, 120));
    }
    resetIzopChatTableEnsure();
    await ensureIzopChatTable();
  }
...
export async function updateIzopChatSession(
  userId: string,
  id: string,
  body: { messages?: unknown; title?: string }
): Promise<IzopChatSessionRow | null> {
  await ensureIzopChatTable();
  ...
  if (explicitTitle && !messages) {
    try {
      if (prismaHasIzopModel()) {
        const row = await prisma.izopChatSession.updateMany({
          where: { id, userId },
          data: { title: explicitTitle },
        });
        if (row.count === 0) return null;
        return getIzopChatSession(userId, id);
      }
    } catch (e) {
      if (!isMissingTableError(e)) throw e;
      resetIzopChatTableEnsure();
  ...
  const existing = await getIzopChatSession(userId, id);
  if (!existing) return null;
  ...
  try {
    if (prismaHasIzopModel()) {
      const row = await prisma.izopChatSession.update({
        where: { id },
        data: {
...
export async function deleteIzopChatSession(userId: string, id: string): Promise<boolean> {
  await ensureIzopChatTable();
  try {
    if (prismaHasIzopModel()) {
      const existing = await prisma.izopChatSession.findFirst({
        where: { id, userId },
        select: { id: true },
      });
      if (!existing) return false;
      await prisma.izopChatSession.delete({ where: { id } });
      return true;
    }
  } catch (e) {
    if (!isMissingTableError(e)) throw e;

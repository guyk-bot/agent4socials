...
describe('linkedin oauth-scopes', () => {
  ...
  const prevPostAnalyticsOff = process.env.LINKEDIN_INCLUDE_R_MEMBER_POST_ANALYTICS;
  ...
  afterEach(() => {
    ...
    if (prevPostAnalyticsOff === undefined) delete process.env.LINKEDIN_INCLUDE_R_MEMBER_POST_ANALYTICS;
    else process.env.LINKEDIN_INCLUDE_R_MEMBER_POST_ANALYTICS = prevPostAnalyticsOff;
  });

  it('returns full personal scopes by default', () => {

...
function getPlatformHint(platform: string): string {
  ...
  if (p === 'TWITTER') return 'X (Twitter) non-premium accounts have a 256 character limit (including spaces). Hashtags are added separately (typically 5, about 40-60 chars). Keep the description to 200-220 characters maximum so there is room for hashtags. Be concise. A clear CTA works well.';

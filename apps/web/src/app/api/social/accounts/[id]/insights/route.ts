...
export async function GET(
  ...
  try {
    ...
    if (account.platform === 'PINTEREST') {
      ...
      // Fallback: when Pinterest daily analytics is unavailable, keep only an overall total
      // from monthly_views. Do NOT fabricate day-level candles from a monthly aggregate.
      ...
      // Keep KPI totals when only monthly aggregate exists, but leave series empty so candles
      // represent only true day-level data returned by Pinterest.
      if ((dailyForBundle?.length ?? 0) === 0 && Number(out.impressionsTotal ?? 0) > 0) {

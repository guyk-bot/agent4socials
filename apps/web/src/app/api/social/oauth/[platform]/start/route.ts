...
function getOAuthUrl(platform: Platform, userId: string, method?: string): string {
  ...
  switch (platform) {
    ...
    case 'FACEBOOK': {
      ...
      return `https://www.facebook.com/v18.0/dialog/oauth?client_id=${process.env.META_APP_ID}&redirect_uri=${encodeURIComponent(fbRedirectUri)}&state=${state}&scope=pages_read_engagement,pages_show_list`;
    }
    case 'TWITTER':

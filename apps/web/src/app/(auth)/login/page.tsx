...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = err && typeof err === 'object' && 'message' in err ? String((err as { message: string }).message) : 'Failed to log in';
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      const message = err && typeof err === 'object' && 'message' in err ? String((err as { message: string }).message) : 'Failed to sign in with Google';
      setError(message);
    } finally {
      setLoading(false);
    }
      ...
      <div className="w-full max-w-md space-y-8 rounded-2xl border border-slate-800 bg-slate-900/50 p-8 shadow-xl">
        ...
        <div className="relative">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-2 bg-slate-900 text-slate-400">or log in with email</span>
          </div>
        </div>

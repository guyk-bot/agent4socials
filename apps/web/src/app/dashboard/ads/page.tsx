...
function GoogleAdsIcon({ size = 32 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" aria-hidden className="shrink-0">
      <path fill="#FBBC04" d="M12 3 4 20h4.5L12 10.5 15.5 20H20L12 3z" />
      <path fill="#34A853" d="M8.5 20 12 12.5 15.5 20H12z" />
      <path fill="#4285F4" d="M4 20h4.5L12 10.5 8.5 20z" />
    </svg>
  );
}
    ...
    <div className="mx-auto w-full max-w-4xl pb-10">
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        {AD_PLATFORMS.map((platform) => (
          <div
            key={platform.id}
            className="flex flex-col rounded-2xl border border-[var(--border)] bg-[var(--card-bg)] p-5 transition-colors hover:border-[var(--color-purple-border)]"
          >
            <div
            ...
            <button
              ...
              className="mt-5 w-full rounded-xl border border-[var(--border)] bg-[var(--background)] px-4 py-2.5 text-sm font-semibold text-[var(--muted)] cursor-not-allowed opacity-80"
            >
              Connect {platform.name.replace(' Ads', '')}
            </button>
            <p className="mt-2 text-center text-[10px] font-semibold uppercase tracking-wide text-[var(--color-purple-text)]">
              Coming soon
            </p>
          </div>
        ))}

...
import api, {
  API_IZOP_CHAT_ATTACHMENTS_TIMEOUT_MS,
  API_IZOP_CHAT_TIMEOUT_MS,
} from '@/lib/api';
...
import { resolveChatBrandContext } from '@/lib/ai/izop-workspace-snapshot';
import {
...
import {
  ...
  validateChatFile,
  type IzopChatAttachment,
} from '@/lib/ai/izop-attachments';
import { IzopArtifactCards } from '@/components/izop/IzopArtifactCards';
import {
  IzopMessageAttachments,
  IzopPendingAttachments,
} from '@/components/izop/IzopMessageAttachments';
import { IzopChatMessageContent } from '@/components/izop/IzopChatMessageContent';
...
export default function IzopChatPanel({
  messages,
  onMessagesChange,
  disabled,
  panelResetKey = 0,
  ...
  const [pendingAttachments, setPendingAttachments] = useState<IzopChatAttachment[]>([]);
  ...
  const handleFilePick = async (files: FileList | null) => {
    ...
    const remaining = IZOP_CHAT_MAX_ATTACHMENTS - pendingAttachments.length;
    if (remaining <= 0) {
      setError(`You can attach up to ${IZOP_CHAT_MAX_ATTACHMENTS} files per message.`);
      return;
    }
  ...
  const runLeadsScan = useCallback(async () => {
    ...
    try {
      ...
      setError(friendlyIzopChatError(e, 'Lead scan failed. Try again.'));
      onMessagesChange(base);
    } finally {
  ...
  const send = useCallback(
    async (text: string, attachments: IzopChatAttachment[] = [], opts?: { resume?: boolean }) => {
      ...
      try {
        ...
        const brandContext = await resolveChatBrandContext({
          ...
          fetchAccounts: async () => {
            const res = await api.get<Array<{ id: string; platform: string; username?: string | null }>>(
              '/social/accounts',
              { signal: ac.signal, timeout: API_IZOP_CHAT_TIMEOUT_MS }
            );
        ...
        const res = await api.post<{ reply: string; artifacts?: IzopArtifact[] }>(
          '/ai/izop-chat',
          apiBody,
        ...
        setError(friendlyIzopChatError(e, 'Something went wrong. Try again.'));
        onMessagesChange(next);
      } finally {
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
              ...
              return (
                <div key={m.id} className="flex justify-end">
                  <div className="flex max-w-[95%] flex-col items-end gap-2">
                    <IzopMessageAttachments attachments={m.attachments!} variant="user" detached />
                    {m.content ? (
                      <div className="rounded-2xl rounded-br-md px-4 py-3 text-sm whitespace-pre-wrap izop-bubble-user">
                        <IzopChatMessageContent content={m.content} variant="user" />
                      </div>
                    ) : null}
                  </div>
                </div>
              ...
              <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div
                  className={`max-w-[95%] rounded-2xl px-4 py-3 text-sm whitespace-pre-wrap ${
                    m.role === 'user'
                      ? 'izop-bubble-user rounded-br-md'
                      : 'izop-bubble-assistant rounded-bl-md shadow-sm'
                  }`}
                >
                  {m.attachments?.length ? (
                    <IzopMessageAttachments attachments={m.attachments} variant={m.role} />
                  ) : null}
                  {m.content ? <IzopChatMessageContent content={m.content} variant={m.role} /> : null}
      ...
      <div className="izop-chat-dock shrink-0">
        <IzopPendingAttachments
        ...
        <button
          ...
          disabled={loading || disabled || uploading || mediaUpload.isUploading || pendingAttachments.length >= IZOP_CHAT_MAX_ATTACHMENTS}
          className="izop-chat-icon-btn-glass shrink-0 rounded-xl px-3 py-3 text-neutral-600 dark:text-neutral-300 disabled:opacity-40 transition-colors"
        ...
        {loading ? (
          <button
            ...
            className="izop-chat-icon-btn-glass shrink-0 rounded-xl text-[var(--foreground)] px-4 py-3 transition-opacity"

...
import { IzopChatVideoPreview } from '@/components/izop/IzopChatVideoPreview';
...
export function IzopMessageAttachments({ attachments, variant = 'user', detached = false }: Props) {
...
export function IzopPendingAttachments({ attachments, onRemove, uploading }: PendingProps) {
  if (!attachments.length) return null;
      ...
      {attachments.map((att, i) => (
        ...
        >
          {att.kind === 'image' ? (
            ...
            <IzopChatVideoPreview src={att.fileUrl} fileName={att.fileName} compact />
          ) : (

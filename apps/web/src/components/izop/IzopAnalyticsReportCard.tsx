...
import type { ReportSnapshotArtifact } from '@/lib/ai/izop-artifacts';
...
export function IzopAnalyticsReportCard({ report }: { report: ReportSnapshotArtifact }) {
  ...
  return (
    <div className="rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 overflow-hidden shadow-sm">
      <div className="px-4 py-3 border-b border-neutral-100 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900/80">
        ...
        <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">{formatRange(report.dateRange)}</p>
      </div>
          ...
          return (
            <button
              ...
              className={`rounded-xl border px-2 py-2 text-left transition-colors ${
                selected
                  ? 'border-[var(--primary)] bg-[#E8F4FF]/60 dark:bg-[var(--primary)]/15'
                  : 'border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800/60 hover:border-neutral-300 dark:hover:border-neutral-600'
              }`}
            >
              <p className="text-[10px] uppercase tracking-wide text-neutral-500 dark:text-neutral-400">{m.label}</p>
      ...
      <div className="px-3 pb-3">
        {hasChartData ? (
          <InteractiveLineChart
            ...
            crosshair
            tooltipStyle={theme === 'dark' ? 'dark' : 'light'}
          />
        ) : (
          <div className="h-[200px] flex items-center justify-center text-sm text-neutral-400 dark:text-neutral-500 rounded-lg border border-dashed border-neutral-200 dark:border-neutral-700">
        ...
        {report.insightsHint ? (
          <p className="text-[11px] text-amber-700 dark:text-amber-400 mt-1">{report.insightsHint}</p>
        ) : null}
      </div>
    </div>
  );

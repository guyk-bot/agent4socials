...
export function PostPerformanceTable({ posts }: PostPerformanceTableProps) {
  ...
  const platformColorCls = (platform: string, active: boolean) => {
    ...
    return active ? 'bg-[#3EE2A7]/20 border-[#3EE2A7]/50 text-[#0b0f1a]' : 'border-slate-200 text-slate-600 hover:bg-[#3EE2A7]/10';
  };
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 max-w-[200px]">
                        ...
                        {post.permalinkUrl && (
                          <a href={post.permalinkUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-[#3EE2A7] hover:underline">
                            Open
                          </a>
                        )}
                      </div>

...
const METRIC_MAP: MetricDef[] = [
  ...
  { key: 'post_impressions_unique', label: 'Unique Reach (legacy)', section: 'posts', source: 'post_impressions_unique', color: COLOR.cyan },
...
/** Prefer Meta `post_total_media_view_unique` when present; fall back to legacy `post_impressions_unique`. */
function bestFacebookPostUniqueViewers(fi: Record<string, number> | undefined): number {
  if (!fi) return 0;
  const next =
    typeof fi.post_total_media_view_unique === 'number' && Number.isFinite(fi.post_total_media_view_unique)
      ? fi.post_total_media_view_unique
      : 0;
  const legacy =
    typeof fi.post_impressions_unique === 'number' && Number.isFinite(fi.post_impressions_unique)
      ? fi.post_impressions_unique
      : 0;
          ...
          {chartData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                />
                {selectedReelMetrics.includes('views') ? <Bar dataKey="views" fill={REEL_METRIC_CONFIG.views.color} radius={[6, 6, 0, 0]} barSize={UNIFIED_BAR_SIZE} shape={<MinWidthBarShape />} isAnimationActive={false} /> : null}
                {selectedReelMetrics.includes('likes') ? <Bar dataKey="likes" fill={REEL_METRIC_CONFIG.likes.color} radius={[6, 6, 0, 0]} barSize={UNIFIED_BAR_SIZE} shape={<MinWidthBarShape />} isAnimationActive={false} /> : null}
...
function classifyPostsUploadBucket(p: FacebookPost, forLinkedIn: boolean): PostsUploadBucket {
  ...
  if (forLinkedIn && (isAnalyticsTextOnlyPost(p) || (p.mediaType ?? '').toUpperCase() === 'TEXT')) return 'text';
  return 'image';
}
...
export function FacebookAnalyticsView({
  ...
  const [selectedTrafficMetrics, setSelectedTrafficMetrics] = useState<TrafficMetricKey[]>([
    'postImpressions',
    'nonviral',
    'viral',
    'uniqueReachProxy',
  ...
  const [selectedPostsUploadTypes, setSelectedPostsUploadTypes] = useState<PostsUploadBucket[]>([
    'reels',
    'image',
    'carousel',
  ]);
  ...
  const contentTypeCounts = useMemo(() => {
    const counts: Record<PostsUploadBucket, number> = { reels: 0, image: 0, carousel: 0, text: 0 };
    for (const p of postsInRangeForPostsTabUi) {
      if (isStoryPost(p)) continue;
      const bucket = classifyPostsUploadBucket(p, isLinkedIn);
      counts[bucket] += 1;
    }
    return counts;
  }, [postsInRangeForPostsTabUi, isLinkedIn]);
  const postsUploadChartPresets = useMemo(() => {
    ...
    if (isLinkedIn) {
      return [
        ...
        { key: 'text' as const, label: 'Text', color: LINKEDIN_UPLOAD_COLOR.text },
        { key: 'reels' as const, label: 'Video', color: LINKEDIN_UPLOAD_COLOR.reels },
        { key: 'image' as const, label: 'Image', color: LINKEDIN_UPLOAD_COLOR.image },
  ...
  const postsUploadByDay = useMemo(() => {
    ...
    for (const d of axis) {
      byDate.set(d, { date: d, reels: 0, image: 0, carousel: 0, text: 0, shorts: 0, videos: 0 });
    }
    for (const p of postsInRangeForPostsTabUi) {
      ...
      if (isStoryPost(p)) continue;
      const bucket = classifyPostsUploadBucket(p, isLinkedIn);
      if (bucket === 'reels') row.reels += 1;
      else if (bucket === 'carousel') row.carousel += 1;
      else if (bucket === 'text') row.text += 1;
      else row.image += 1;
    }
  ...
  const postsUploadChartYMax = useMemo(() => {
    ...
    for (const row of postsUploadByDay) {
      const stacked =
        Number(row.reels) + Number(row.image) + Number(row.carousel) + Number(row.text ?? 0);
      max = Math.max(
        max,
        stacked,
        Number(row.reels),
  ...
  const uniqueReachProxy = postsInRange.reduce(
    (s, p) => s + bestFacebookPostUniqueViewers(p.facebookInsights as Record<string, number> | undefined),
    0
  );
  ...
  const uniqueReachByDate = useMemo(() => {
    ...
    postsInRange.forEach((p) => {
      const d = localCalendarDateFromIso(p.publishedAt);
      if (!d) return;
      map[d] = (map[d] ?? 0) + bestFacebookPostUniqueViewers(p.facebookInsights as Record<string, number> | undefined);
    });
    return map;
  }, [postsInRange]);
...
type PostsUploadDayTooltipAgg = {
  ...
  const postsUploadTooltipByDate = useMemo(() => {
    ...
    const map = new Map<
      string,
      {
        all: PostsUploadDayTooltipAgg;
        reels: PostsUploadDayTooltipAgg;
        image: PostsUploadDayTooltipAgg;
        carousel: PostsUploadDayTooltipAgg;
        text: PostsUploadDayTooltipAgg;
      }
    >();
    for (const r of postsRows) {
      const d = toLocalCalendarDate(new Date(r.date));
      if (!map.has(d)) {
        map.set(d, { all: empty(), reels: empty(), image: empty(), carousel: empty(), text: empty() });
      }
      const row = map.get(d)!;
      ...
      const apply = (agg: PostsUploadDayTooltipAgg) => {
        ...
        if (!agg.items.some((x) => x.id === r.id)) {
          agg.items.push({
            id: r.id,
            thumbnailUrl: u || null,
            mediaFormat: analyticsPostTypeLabel(
              r.rawPost,
              r.type === 'Story' ? 'Story' : r.type === 'Reel' ? 'Reel' : 'Post'
            ),
  ...
  const youtubeAllVideoRows = useMemo((): ReelAnalyticsRow[] => {
    if (!isYouTube) return [];
    return postsInRange
      .filter((p) => (p.platform ?? '').toUpperCase() === 'YOUTUBE')
      .map((p) => {
        const { watchTimeMs, avgWatchMs } = getWatchTimes(p);
        return {
          post: p,
  ...
  /** Pinterest / Twitter / YouTube all-video rows; other platforms use reel rows only. */
  const reelChartSourceRows = useMemo((): ReelAnalyticsRow[] => {
    ...
    if (isYouTube) return youtubeAllVideoRows;
    return reelsRows;
  }, [isPinterest, pinterestVideoRows, isTwitter, twitterVideoReelsRows, isYouTube, reelsRows, youtubeAllVideoRows]);
  ...
  return (
    <div className="analytics-dark-scope p-0 md:p-0.5 space-y-3" style={{ background: COLOR.pageBg, maxWidth: 1400 }}>
      {onUpgrade ? (
        <div className="w-full rounded-2xl upgrade-banner-warm px-3 py-2.5 sm:px-4 sm:py-3 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5 sm:gap-3">
        ...
        <div className="rounded-[20px] border p-4 sm:p-5 space-y-3" style={{ borderColor: COLOR.border, background: COLOR.card, boxShadow: '0 4px 22px rgba(15,23,42,0.06)' }}>
          ...
          >
            ...
            {isPinterest ? (
              <MetricCard
                label="Engagements"
                source="Pinterest user analytics · ENGAGEMENT (or SAVE + OUTBOUND_CLICK + PIN_CLICK fallback)"
                color={COLOR.cyan}
              ...
              <div className="rounded-xl border p-4 sm:p-5" style={{ borderColor: COLOR.border, background: COLOR.sectionAlt }}>
                ...
                {youtubeGeoBreakdown.pieSlices.length > 0 ? (
                  <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                    ...
                    <ul className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3 text-sm min-w-0" style={{ color: COLOR.text }}>
                      {youtubeGeoBreakdown.list.map((row, i) => (
                        <li
                          key={`${row.name}-${i}`}
                          className="grid min-w-0 grid-cols-[10px_minmax(0,1fr)_auto] items-center gap-x-2 border-b border-neutral-100 py-2"
                        >
          ...
          <div className="rounded-xl border p-4 sm:p-5" style={{ borderColor: COLOR.border, background: COLOR.sectionAlt }}>
            ...
            <div className="grid w-full grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">
              {postsUploadChartPresets
                ...
                .map((preset) => {
                  const key = preset.key as PostsUploadBucket;
                            ...
                            <div className="mb-2 space-y-0.5" style={{ color: COLOR.textSecondary }}>
                              {(payload ?? [])
                                .filter((x) => {
                                  ...
                                  if (
                                    !Number(x.value) ||
                                    (dk !== 'reels' && dk !== 'image' && dk !== 'carousel' && dk !== 'text')
                                  ) {
                                    return false;
                                  }
                                  return postsUploadVisibleKeys.includes(dk as PostsUploadBucket);
                                })
                                .map((x) => (
            ...
            >
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  />
                  <Bar dataKey="views" fill={REEL_METRIC_CONFIG.views.color} radius={[6, 6, 0, 0]} barSize={UNIFIED_BAR_SIZE} shape={<MinWidthBarShape />} isAnimationActive={false} />
                  <Bar dataKey="likes" fill={REEL_METRIC_CONFIG.likes.color} radius={[6, 6, 0, 0]} barSize={UNIFIED_BAR_SIZE} shape={<MinWidthBarShape />} isAnimationActive={false} />
                  <Bar dataKey="comments" fill={REEL_METRIC_CONFIG.comments.color} radius={[6, 6, 0, 0]} barSize={UNIFIED_BAR_SIZE} shape={<MinWidthBarShape />} isAnimationActive={false} />
                  <Bar dataKey="shares" fill={REEL_METRIC_CONFIG.shares.color} radius={[6, 6, 0, 0]} barSize={UNIFIED_BAR_SIZE} shape={<MinWidthBarShape />} isAnimationActive={false} />
                </BarChart>
              </ResponsiveContainer>
            </InsightChartCard>
          ) : null}

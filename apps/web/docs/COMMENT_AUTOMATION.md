...
- A **cron job** runs every **2 minutes** (Vercel Cron: `/api/cron/comment-automation`). It finds replies/comments on those posts and, if the comment text contains one of your keywords, posts your auto-reply **once per comment**.
...
2. **Cron hasn’t run yet**  
   The job runs every 2 minutes. After you reply, wait up to about 2 minutes and check again.

...
| Facebook Graph raw JSON debug (read_insights, page, posts, etc.); `?full=1` = full insights bodies for all candidate metrics + post lifetime on first published post | `apps/web/src/app/api/social/accounts/[id]/facebook-graph-debug/route.ts` |
